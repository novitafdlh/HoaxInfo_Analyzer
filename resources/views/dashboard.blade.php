        ...
        <div class="flex items-center gap-4 pb-3 border-b border-blue-100">
            ...
            <div>
                <h2 class="font-extrabold text-2xl text-slate-900 tracking-tight leading-tight">
                    {{ __('Dashboard Validasi Publik') }}
                </h2>
                <p class="text-xs font-medium text-blue-700 uppercase tracking-widest mt-0.5">Analisis kemiripan konten bantuan sosial</p>
            </div>
        </div>
    </x-slot>
                ...
                <aside class="lg:col-span-4 space-y-6">
                    <div class="rounded-3xl border border-blue-100 bg-white p-8 shadow-lg shadow-blue-50/50 transition-all duration-300 hover:shadow-blue-100/50 hover:-translate-y-1">
                        ...
                        <p class="text-sm text-slate-600 leading-relaxed mb-6">
                            Unggah gambar atau tempelkan URL untuk membandingkan konten dengan basis data resmi menggunakan pencocokan hash, OCR, dan cosine similarity.
                        </p>
                        ...
                        <ul class="space-y-4 text-xs text-slate-600">
                            <li class="flex items-start gap-3">
                                ...
                                <div class="flex-1"><b class="text-slate-700">Hash SHA256 identik:</b> Sistem memberi confidence sangat tinggi dan menandai konten sebagai terverifikasi otomatis.</div>
                            </li>
                            <li class="flex items-start gap-3">
                                ...
                                <div class="flex-1"><b>Similarity 60% - 84%:</b> Konten mirip, tetapi tetap perlu validasi manual admin.</div>
                            </li>
                            <li class="flex items-start gap-3">
                                ...
                                <div class="flex-1"><b>Similarity di bawah 60%:</b> Sistem hanya memberi peringatan, bukan vonis mutlak. Admin tetap melakukan peninjauan lanjutan.</div>
                            </li>
                        </ul>
                        ...
                        @else
                            <div class="mt-8 space-y-4 border-t border-blue-100 pt-6">
                                <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-inner shadow-amber-100/40">
                                    @if ($guestUploadLimitEnabled)
                                        <p class="text-xs font-bold uppercase tracking-[0.24em] text-amber-700">Token Gratis Harian</p>
                                        <p class="mt-3 text-3xl font-black text-amber-900">{{ $guestUploadsRemaining }}</p>
                                        <p class="mt-1 text-sm font-medium text-amber-800">sisa dari {{ $guestUploadLimit }} validasi gratis hari ini</p>
                                    @else
                                        <p class="text-xs font-bold uppercase tracking-[0.24em] text-emerald-700">Mode Testing Lokal</p>
                                        <p class="mt-3 text-3xl font-black text-emerald-900">Tanpa Batas</p>
                                        <p class="mt-1 text-sm font-medium text-emerald-800">Token guest sedang dinonaktifkan untuk memudahkan proses testing.</p>
                                    @endif
                                </div>
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                                    <p class="text-sm font-semibold text-slate-900">Pemakaian hari ini</p>
                                    @if ($guestUploadLimitEnabled)
                                        <p class="mt-2 text-sm text-slate-600">{{ $guestUploadsUsed }} dari {{ $guestUploadLimit }} token sudah digunakan.</p>
                                        <p class="mt-2 text-xs leading-relaxed text-slate-500">
                                            Untuk guest, sistem membatasi maksimal {{ $guestUploadLimit }} upload per hari. Login diperlukan jika ingin upload tanpa batas.
                                        </p>
                                    @else
                                        <p class="mt-2 text-sm text-slate-600">{{ $guestUploadsUsed }} upload guest sudah dilakukan hari ini.</p>
                                        <p class="mt-2 text-xs leading-relaxed text-slate-500">
                                            Pembatas guest sedang dimatikan pada environment lokal. Saat hosting, aktifkan lagi agar penggunaan publik tetap terkontrol.
                                        </p>
                                    @endif
                                </div>
                            </div>
                ...
                <main class="lg:col-span-8">
                    <div class="rounded-3xl border border-blue-100 bg-white p-8 shadow-xl shadow-blue-100/30 transition-all duration-300 hover:shadow-blue-100/50">
                        ...
                        @unless ($isAuthenticated)
                            @if ($guestUploadLimitEnabled)
                                <div class="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900 shadow-sm shadow-amber-100/50">
                                    <div class="font-bold">Guest mendapat {{ $guestUploadLimit }} token gratis per hari.</div>
                                    <div class="mt-1">Sisa token Anda hari ini: {{ $guestUploadsRemaining }}.</div>
                                </div>
                            @else
                                <div class="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-900 shadow-sm shadow-emerald-100/50">
                                    <div class="font-bold">Mode testing aktif: token guest tidak dipakai.</div>
                                    <div class="mt-1">Anda bisa upload berulang kali tanpa batas selama masih di environment lokal/testing.</div>
                                </div>
                            @endif
                        @endunless
                        ...
                        <form method="POST" action="{{ route('dashboard.upload') }}" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="image_url" name="image_url" type="url" :value="old('image_url')" placeholder="https://domain.com/gambar-bansos.jpg" class="mt-2 block w-full rounded-xl border-blue-200 bg-white py-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-150" />
                    ...
                    <div class="flex items-start justify-between gap-6">
                        <div>
                            ...
                            <p class="mt-2 text-sm leading-relaxed text-slate-600">
                                Sistem menampilkan hasil analisis awal agar guest tetap langsung tahu apakah konten perlu diwaspadai atau sudah terverifikasi.
                            </p>
                        </div>
